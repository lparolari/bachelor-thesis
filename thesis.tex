...
\item \textbf{I}ntuitive. Picat fornisce comandi come l'assegnamento
  e i loop, che vengono comunemente usati nei linguaggi imperativi,
...
logici, obbligando però il programmatore a usarle in modo esplicito,
...
vincolata a un valore (operazione di bounding), e può avere degli
attributi rappresentati come mappa chiave-valore.\footnote{Gli
  ...
  vincoli integrati nei moduli predefiniti di Picat per memorizzare
...
I tipi composti invece sono le liste $[t_1, \ldots,
t_n]$ e le strutture $\$s(t_1, \ldots, t_n)$, dove
...
I predicati sono una componente fondamentale di Picat: essi
definiscono una relazione che può avere zero, una o più soluzioni,
terminando in modo positivo o negativo, a meno di eccezioni.
...
Un predicato è detto essere \emph{deterministico} se è definito
solamente da regole non-backtrackable, \emph{non-deterministico}
invece se nella definizione del predicato compare almeno una regola
backtrackable. Inoltre, si dice \emph{globalmente deterministico} un
predicato deterministico che in $Body$ contiene solo chiamate a
predicati globalmente deterministici. Per questo motivo, un
predicato deterministico può avere più di una soluzione, mentre
uno globalmente deterministico al massimo una.
...
valori di altre list. La sintassi per esprimerle è la seguente
\begin{lstlisting}
...
chiamata a un predicato e relativo risultato per evitare cicli
...
la dichiarazione del modulo stesso, esso è assunto appartenere al
...
\begin{verbatim}
%%% file pericolo.pi
pericolo => X = 5 // 0.   % divisione per zero, solleva
...
delle eccezioni, l'esecuzione e altre utilità.
...
event-driven. Un attore è un chiamata a un predicato che può essere
differita rispetto alla normale esecuzione ed è attivabile da
eventi. Queste regole hanno sintassi 
...
notificare eventi e può avere diverse porte, che specificano
la visibilità dell'evento per quel canale.

\subsection{Mappe Globali}
\label{subsec:picat_advanced_globalmaps}

Picat fornisce tre tipi diversi di mappe utilizzabili durante
l'esecuzione: mappe \emph{heap}, mappe \emph{globali} e
\emph{tabelle}.

Queste mappe sono create non appena il sistema Picat si avvia. 
Possono essere consultate e modificate richiedendone un riferimento
al sistema, specificando tipologia e codice identificativo.
Le tre mappe differiscono per la modalità di memorizzazione
degli elementi e per il numero di mappe che possono essere richieste.
...
\item[Unificazione e pattern-matching.] L'unificazione è l'operazione
  ...
  impiegato il pattern-matching per decidere se una regola è
...
\item[Backtracking.] Anche Picat supporta nativamente il
  ...
  \emph{backtrackable} e si usa l'operatore \emph{cut} (\texttt{!}) 
  per controllarne il comportamento. In Picat invece una regola è
...
funzionamento e i predicati implementati. La sezione
...
L'organizzazione del progetto e dei file sorgente è di particolare
rilevanza: permette una miglior stesura del codice, facilità
di lettura e manutenzione, creazione dei test e consente a futuri
collaboratori di lavorare sul software in modo ordinato e preciso.
Soprattutto nell'ambito dei linguaggi logici esistono
...
\textit{tightly coupled}\footnote{Letteralmente, \textit{tightly
  ...
  e interconnessione.}, mentre le funzionalità \textit{loosely
...
$neq(\cdot, \cdot)$, $\in$ diventa $in(\cdot, \cdot)$\footnote{Più
  precisamente, in Picat questo simbolo è stato
  trasformato in \texttt{insl}, in quanto il predicato \texttt{in} è presente
...
\begin{itemize}
	...
	\item \textbf{Rappresentazione ``lista-resto''} La
          ...
          lista, e il resto dell'insieme corrisponde al resto della
          lista.
\end{itemize}
...
% entrambi i costruttori creano un insieme costituito da
%  testa [1, 2] e resto A.
Picat> Y = new_lset([1,2|A])
Picat> Z = new_lset([1,2], A)


% restituisce in YHead il resto dell'insieme Y
Picat> YHead = Y.lset_head()
YHead = [1,2]

% restituisce in YRest il resto dell'insieme Y
...
una CLI (Command Line Interface) orientata all'usabilità utente:
fornisce messaggi di aiuto, cerca di effettuare controlli 
sull'input e una sapiente gestione degli errori.

Di fatto questo modulo implementa un ciclo che non termina mai se non
quando il comando di uscita è ricevuto. Inoltre, il ciclo principale è
racchiuso nel predicato di ordine superiore \texttt{catch} (si veda
...
\begin{enumerate}[label=\arabic*)]
	...
	\item mostra un messaggio di introduzione contenente qualche
          piccola nota sulla garanzia, copyright e mostra i comandi di
...
programmatore a effettuare operazioni sulle variabili globali senza
...
\begin{itemize}
	...
	\item \texttt{set\textunderscore global/2}, prende in input
          una chiave \emph{key} e un valore \emph{value} e imposta in
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \texttt{all}, comprende tutti i livelli di
                  verbosità.
	\end{itemize}
	I tre livelli di verbosità predefiniti sono volutamente
        implementati con ereditarietà (e.g., il livello di verbosità
        \texttt{vvv} mostra i log anche dei livelli \texttt{vv} e
        \texttt{v} oltre ai propri). Comunque, il modulo permette di
	...
	\item \textbf{Tipi di log}. I log possono avere tipi
          ...
          per distinguere a colpo d'occhio il tipo dei log. A ogni
...
\begin{itemize}
	...
	\item \texttt{log\textunderscore d/2},
          ...
          log) sia tra quelli impostati e in caso lanciano la
          scrittura del log;
...
\begin{itemize}
	\item \texttt{prompt(Q) = A}. Mostra la domanda \texttt{Q} su
	      \texttt{stdout} all'utente e ne cattura una risposta arbitraria \texttt{A} da \texttt{stdin}.
	      Attualmente non implementato, solleva
          \texttt{not\textunderscore implemented\textunderscore
            exception} (predisposto nell'interfaccia per utilizzi
            futuri).
...
\begin{figure}
	...
	test() =>
	    ...
	    % dell'asserzione o all'esecuzione di metodi 
...
risultato può avere zero, una o più soluzioni e il solver deve essere
...
\end{itemize}
La formula finale ottenuta è semplificata, irriducibile ed equivalente alla forma originale.
...
nell'effettuare gli \emph{unit tests}. L'esempio riportato, seppur poco
...
trovare tutte (o alcune) soluzioni a un certo problema
...
\begin{itemize}
	\item la prima regola gestisce il caso in cui la lista di
          clausole in or sia vuota, e ha sempre successo;
...
che prendono in esame casi semplici ma cercano di esplorare i casi
limite.
...
con un occhio di riguardo alla leggibilità e alla semplicità del
...
disgiunzione, dove il fallimento della regola è contemplato, e non
...
interazione da linea di comando (possibili evoluzioni potrebbero
sostituire la CLI con una interfaccia utente più evoluta, come 
una GUI).
Questa modalità è utilizzata per scopi fine a se stessi, come il
...
Questi step nella pratica si traducono in
\begin{verbatim}
$ Picat
picat> load(main.pi)
...
picat> main.
\end{verbatim}
oppure, grazie a una forma più concisa, in 
\texttt{picat main.pi} (vedere
...
\begin{itemize}
	...
	\item \texttt{quiet}, imposta la modalità silenziosa per le
          interazioni utente.
\end{itemize}
...
\begin{itemize}
	\item il livello di log da visualizzare;
	\item il file su cui scrivere i log;
	\item di visualizzare o meno i messaggi del solver (ad esempio, 
	      i passaggi delle riscritture).
\end{itemize}

Configurazioni di basso livello sul sistema o sul solver
sono proibite o determinate a tempo di compilazione, all'utente
non è quindi permessa la modifica.

Un esempio di configurazione è mostrato in figura
\ref{fig:example_confcommands}.
...
\begin{figure}
	...
	log>
	\end{verbatim}
	\caption{Esempio: comando solve (e varianti) della modalità interattiva}
...
progettazione e implementazione in Picat di un constraint solver per
...
formule di \lset{} adottata in \setlog{} a causa delle citate
limitazioni di Picat rispetto al Prolog sulla forma sintattica dei
termini.  Si è quindi utilizzato per il momento una sintassi
nettamente più vicina alla rappresentazione interna di Picat. La
soluzione a questo problema è riassumibile dallo schema seguente
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
	...
	\end{tikzpicture}
\end{figure}
che aggiunge un livello: il parsing delle formule \lset{}.  In questo
modo il solver non ha bisogno di modifiche: le formule manipolate sono
sempre rappresentate con una sintassi vicina a quella di Picat, ma
l'utente (sia umano che non) può lavorare con formule \lset{}.  Così
facendo, inoltre, si può decidere di realizzare questo livello
intermedio in un linguaggio diverso da Picat. Possiamo citare Prolog,
ad esempio, con il modulo DCG \cite{MetalevelDCG} (Definite Clause
Grammar) che consente di effettuare il parsing di frasi dando la
...
era obiettivo di questo lavoro di tesi, ma si è reso necessario per
capire, almeno indicativamente, l'aderenza dell'implementazione alla
teoria. I casi esaminati non sono ovviamente esaustivi e non coprono
nemmeno tutti i casi limite che andrebbero verificati.
...
Vorrei spendere qualche parola per ringraziare tutti coloro che durante
...
Mio papà, che con la sua franchezza e i suoi (strani) motti mi ha
sempre spronato.

I miei due fratellini, che con la loro leggerezza hanno saputo farmi
ridere anche nei momenti più bui.

Le mie nonne, con la loro cura e apprensione e mio nonno, con la sua
...
Ringrazio Antonio e Silvio, per i loro preziosi consigli e
l'accompagnamento dal punto di vista professionale e lavorativo.

\bigskip

Ringrazio tutti quelli che in un modo o nell'altro mi hanno dedicato
un aiuto, un ``buona fortuna'' a un esame o una risata,
